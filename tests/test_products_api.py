...
async def test_get_products_by_model_sku(client, seed_products):
    resp = await client.get("/api/products", params={"sku": "CHILD-SKU-1A"})
    assert resp.status_code == 200
    data = resp.json()
    assert data["count"] == 1
...
async def test_get_products_multiple_skus(client, seed_products):
    resp = await client.get("/api/products", params={"sku": "PARENT-SKU-1,PARENT-SKU-2"})
    assert resp.status_code == 200
    data = resp.json()
    assert data["count"] == 2
...
async def test_get_products_not_found(client, seed_products):
    resp = await client.get("/api/products", params={"sku": "NONEXISTENT"})
    assert resp.status_code == 200
    data = resp.json()
    assert data["count"] == 0
